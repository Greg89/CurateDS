...
Original P1 refactors are largely **complete**:

- API endpoint decomposition (P1-A) — done.
- Shared item attribute validator (P1-B) — done.
- `ItemQueryBuilder` extraction (P1-C) — done.
- Web API split + zod validation (P1-D) — done.
- `ItemsPage` workflow extraction (P1-E) — done.
- Standardized `ApiResponses.NotFound` usage (P1-F) — done.
- Bucket provisioning lifted out of upload path (P2-A, code-bug half) — done.

Still open: mobile typecheck reproducibility (P0-1), media-privacy product
decision (P2-A), transaction boundaries (P2-B), collector domain models
(P2-C → roadmap M3), web design system (P2-D), perf hardening (P3), and the
five gap findings (A–E) from `04-cross-reference-and-assessment.md`.
